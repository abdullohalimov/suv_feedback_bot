...
async def user_start(callback: CallbackQuery, callback_data: Factories.Language, state: FSMContext):
    ...
    await callback.message.edit_text(_("Sertifikatni yuklab olish uchun quyidagi so'rovnomani to'ldiring."))
    await callback.message.answer(_('Universitet professor-o\'qituvchisini baholang\n(Suv tejovchi texnologiyalarning afzalliklari va ularni samaradorligi)'), reply_markup=await score_keyboard(1))
...
async def first_step(callback: CallbackQuery, callback_data: Factories.Score, state: FSMContext):
    await callback.message.edit_text(_("Turk mutaxassisini baholang\n(Zamonaviy sug'orish tizimining ahamiyati va suvdan foydalanish madaniyati)"), reply_markup=await score_keyboard(2))
...
async def second(callback: CallbackQuery, callback_data: Factories.Score, state: FSMContext):
    await callback.message.edit_text(_("Bank mutaxassisini baholang\n(Iqtisodiy va huquqiy savodxonlik: subsidiya, kafilliklar, soliq imtiyozlari va bank kreditlari)"), reply_markup=await score_keyboard(3))
...
async def third(callback: CallbackQuery, callback_data: Factories.Score, state: FSMContext):
    await callback.message.edit_text(_("Suv xo'jaligi vazirligi mutaxassisini baholang\n(Iqtisodiy va huquqiy savodxonlik: subsidiya, kafilliklar, soliq imtiyozlari va bank kreditlari)"), reply_markup=await score_keyboard(4))
...
async def four(callback: CallbackQuery, callback_data: Factories.Score, state: FSMContext):
    await callback.message.edit_text(_("O'quv kursi tashkiliy jarayonlarini baholang\n(o'quv materiallari, esdalik sovg'alar, tushlik va boshqalar)"), reply_markup=await score_keyboard(5))
...
async def five(callback: CallbackQuery, callback_data: Factories.Score, state: FSMContext):
    ...
    await callback.message.edit_text(_("O'quv kursi haqida fikrlaringiz bo'lsa, shu yerda yozib qoldiring (majburiy emas)"), reply_markup=await continue_step(data['language']))
...
async def seven(message: Message, state: FSMContext):
    ...
    if re.match(digits_regexp, message.text):
        ...
        if request:
            ...
            await message.answer(_('Boshqa sertifikat yuklash uchun /start komandasini yozing va so\'rovnomani qayta to\'ldiring'))
        ...
        await message.answer(_('ID raqami notog\'ri kiritildi! Iltimos, tekshirib qayta kiritng.'))    